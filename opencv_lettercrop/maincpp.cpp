...

{
	...
	if (contours.size() > 0) {
		...
		for (int i = 0, j = 0; i < contours.size(); i++) {
			...
			if (hierarchy[i][3] == hier) {
				...
				//rectangle(binary_image, Point(roiRect[j].x, roiRect[j].y), Point(roiRect[j].x + roiRect[j].width, roiRect[j].y + roiRect[j].height), Scalar(0, 0, 255), 7); //디버깅
				j++;
			}
		}
	}
	...
	//저장
	for (int i = 0; i < 10; i++) {
		Mat roi = binary_image(roiRect[i]); //CROP
	...
	//출력
	printWindow("binary_image 이미지", binary_image);
	printWindow("result_binary_image 이미지", result_binary_image);
	printWindow("input_origin_image", input_origin_image);
